...
Create a class SubstitutionCipher that implements the interface MessageEncoder, as described above. 
...
Create a class ShuffleCipher that implements the interface MessageEncoder, as described above. 
The constructor should have one parameter called numberOfShuffle. 
Define the method encode so that the message is shuffled numberOfShuffle times. 
To perform one shuffle, split the message in half and then take characters from each half alternately. 
For example, if the message is abcdefghi, the halves are abcde and fghi. The shuffled message is afbgchdie. 
